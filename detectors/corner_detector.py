...
class CornerDetector(Detector):
    def __init__(
        self,
        model_path: str = "./model_weights/cornerDetector.onnx",
        retain_factor: float = 0.85,
    ) -> None:
        super().__init__()
    ...
    def __call__(self, image: np.ndarray) -> Tuple[int]:
        ...
        while image.shape[0] > 10 and image.shape[1] > 10:
            ...
            start_x = (
                min(
                    x_loc + int(round(image.shape[1] * self.retain_factor / 2)),
                    image.shape[1],
                )
                - int(round(image.shape[1] * self.retain_factor))
                if x_loc > image.shape[1] / 2
                else max(x_loc - int(image.shape[1] * self.retain_factor / 2), 0)
            )

            start_y = (
                min(
                    y_loc + int(image.shape[0] * self.retain_factor / 2), image.shape[0]
                )
                - int(image.shape[0] * self.retain_factor)
                if y_loc > image.shape[0] / 2
                else max(y_loc - int(image.shape[0] * self.retain_factor / 2), 0)
            )
            ...
            image = image[
                start_y : start_y + int(image.shape[0] * self.retain_factor),
