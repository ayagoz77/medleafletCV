...
class DocumentDetector(Detector):
    def __init__(
        self,
        model_path: str = "./model_weights/docDetector.onnx",
    ) -> None:
        super().__init__()
    ...
    def __call__(self, image: np.ndarray) -> Tuple[Tuple[np.ndarray]]:
        ...
        prediction = self.model.run(None, {self.model.get_inputs()[0].name: img})[0][0]
        ...
        top_right = image[
            ...
            int((x_cords[1] + x_cords[0]) / 2) : min(
                image.shape[1] - 1, int(x_cords[1] + (x_cords[1] - x_cords[0]) / 2)
            ),
        ]

        bottom_right = image[
            int((y_cords[1] + y_cords[2]) / 2) : min(
                image.shape[0] - 1, int(y_cords[2] + (y_cords[2] - y_cords[1]) / 2)
            ),
            int((x_cords[2] + x_cords[3]) / 2) : min(
                image.shape[1] - 1, int(x_cords[2] + (x_cords[2] - x_cords[3]) / 2)
            ),
        ]

        bottom_left = image[
            int((y_cords[0] + y_cords[3]) / 2) : min(
                image.shape[0] - 1, int(y_cords[3] + (y_cords[3] - y_cords[0]) / 2)
            ),
